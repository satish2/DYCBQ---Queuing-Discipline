...
/*
#define TCF_CBQ_LSS_LIMIT	1200
#define TCF_CBQ_LSS_VARIABLESPACE	700
#define TCF_CBQ_LSS_TOTALSHAREDSPACE	1400
*/
	__u32		maxidle;
	__u32		minidle;
	__u32		offtime;
...
{
	fprintf(stderr, "Usage: ... dycbq rate BPS [ avpkt BYTES ]\n");
	fprintf(stderr, "               [ allot BYTES ]\n");
	fprintf(stderr, "               [ prio NUMBER ]\n");
	fprintf(stderr, "               [limit PKTS] [variableSpace PKTS]\n");
}

static void explain(void)
{
	fprintf(stderr, "Usage: ... dycbq bandwidth BPS avpkt BYTES \n");
	fprintf(stderr, "               [totalSharedSpace PKTS]\n");
}
...
static int dycbq_parse_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	struct tc_ratespec r;
	struct tc_dycbq_lssopt lss;
	__u32 rtab[256];

	unsigned mpu=0, avpkt=0, allot=0;
	__u32 totalSharedSpace; //limit,variableSpace,
	...
	while (argc > 0) {
		...
		} else if (matches(*argv, "totalSharedSpace") == 0) {
			NEXT_ARG();
			if (get_u32(&totalSharedSpace, *argv, 0)) {
				explain1("totalSharedSpace");
				return -1;
			}
	...
	if (r.rate == 0) {
		fprintf(stderr, "DYCBQ: bandwidth is required parameter.\n");
		return -1;
	}
	if (avpkt == 0) {
		fprintf(stderr, "DYCBQ: \"avpkt\" is required.\n");
		return -1;
	}
	if (allot < (avpkt*3)/2)
		allot = (avpkt*3)/2;
	...
	if (tc_calc_rtable(&r, rtab, cell_log, allot, linklayer) < 0) {
		fprintf(stderr, "DYCBQ: failed to calculate rate table.\n");
		return -1;
	}
	...
	lss.avpkt = avpkt;
/*	lss.limit = limit;
	lss.variableSpace = variableSpace;*/
	lss.totalSharedSpace = totalSharedSpace;
//	fprintf(stdout,"totalSharedSpace = %d",totalSharedSpace);

	tail = NLMSG_TAIL(n);
	addattr_l(n, 1024, TCA_OPTIONS, NULL, 0);
	addattr_l(n, 1024, TCA_DYCBQ_RATE, &r, sizeof(r));
	addattr_l(n, 1024, TCA_DYCBQ_LSSOPT, &lss, sizeof(lss));
	addattr_l(n, 3024, TCA_DYCBQ_RTAB, rtab, 1024);
	if (show_raw) {
		int i;
		for (i=0; i<256; i++)
			printf("%u ", rtab[i]);
...
static int dycbq_parse_class_opt(struct qdisc_util *qu, int argc, char **argv, struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (matches(*argv, "rate") == 0) {
			...
			NEXT_ARG();
			if (get_u32(&lss.limit, *argv,0)) {
				explain1("limit");
				return -1;
			}

		} else if (matches(*argv, "variableSpace") == 0) {
			NEXT_ARG();
			if (get_u32(&lss.variableSpace, *argv,0)) {
				explain1("limit");
				return -1;
			}
	...
	if (lss.change&(TCF_CBQ_LSS_OFFTIME|TCF_CBQ_LSS_MAXIDLE)) {
		if (lss.avpkt == 0) {
			fprintf(stderr, "DYCBQ: avpkt is required for max/minburst.\n");
			return -1;
		}
		if (bndw==0 || r.rate == 0) {
			fprintf(stderr, "DYCBQ: bandwidth&rate are required for max/minburst.\n");
			return -1;
		}
	}
	...
	if (wrr_ok) {
		...
		if (wrr.allot == 0) {
			fprintf(stderr, "DYCBQ: \"allot\" is required to set WRR parameters.\n");
			return -1;
		}
	}
	...
	if (wrr_ok)
		addattr_l(n, 1024, TCA_DYCBQ_WRROPT, &wrr, sizeof(wrr));
	if (fopt_ok)
		addattr_l(n, 1024, TCA_DYCBQ_FOPT, &fopt, sizeof(fopt));
	if (r.rate) {
		addattr_l(n, 1024, TCA_DYCBQ_RATE, &r, sizeof(r));
		addattr_l(n, 3024, TCA_DYCBQ_RTAB, rtab, 1024);
		if (show_raw) {
...
static int dycbq_print_opt(struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
	struct rtattr *tb[TCA_DYCBQ_MAX+1];
	struct tc_ratespec *r = NULL;
	struct tc_dycbq_lssopt *lss = NULL;
	...
	parse_rtattr_nested(tb, TCA_DYCBQ_MAX, opt);

	if (tb[TCA_DYCBQ_RATE]) {
		if (RTA_PAYLOAD(tb[TCA_DYCBQ_RATE]) < sizeof(*r))
			fprintf(stderr, "DYCBQ: too short rate opt\n");
		else
			r = RTA_DATA(tb[TCA_DYCBQ_RATE]);
	}
	if (tb[TCA_DYCBQ_LSSOPT]) {
		if (RTA_PAYLOAD(tb[TCA_DYCBQ_LSSOPT]) < sizeof(*lss))
			fprintf(stderr, "DYCBQ: too short lss opt\n");
		else
			lss = RTA_DATA(tb[TCA_DYCBQ_LSSOPT]);
	}
	if (tb[TCA_DYCBQ_WRROPT]) {
		if (RTA_PAYLOAD(tb[TCA_DYCBQ_WRROPT]) < sizeof(*wrr))
			fprintf(stderr, "DYCBQ: too short wrr opt\n");
		else
			wrr = RTA_DATA(tb[TCA_DYCBQ_WRROPT]);
	}
	if (tb[TCA_DYCBQ_FOPT]) {
		if (RTA_PAYLOAD(tb[TCA_DYCBQ_FOPT]) < sizeof(*fopt))
			fprintf(stderr, "DYCBQ: too short fopt\n");
		else
			fopt = RTA_DATA(tb[TCA_DYCBQ_FOPT]);
	}
	if (tb[TCA_DYCBQ_OVL_STRATEGY]) {
		if (RTA_PAYLOAD(tb[TCA_DYCBQ_OVL_STRATEGY]) < sizeof(*ovl))
			fprintf(stderr, "DYCBQ: too short overlimit strategy %u/%u\n",
				(unsigned) RTA_PAYLOAD(tb[TCA_DYCBQ_OVL_STRATEGY]),
				(unsigned) sizeof(*ovl));
		else
			ovl = RTA_DATA(tb[TCA_DYCBQ_OVL_STRATEGY]);
	}
...
static int dycbq_print_xstats(struct qdisc_util *qu, FILE *f, struct rtattr *xstats)
{
...
struct qdisc_util dycbq_qdisc_util = {
	.id		= "dycbq",
	.parse_qopt	= dycbq_parse_opt,
	.print_qopt	= dycbq_print_opt,
	.print_xstats	= dycbq_print_xstats,
	.parse_copt	= dycbq_parse_class_opt,
	.print_copt	= dycbq_print_opt,
};
